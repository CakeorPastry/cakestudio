...
function sanitizeUsername(username) {
    username = username.normalize('NFD').replace(/[\u0300-\u036f]/g, ''); // Normalize and remove accents
    return username.replace(/[^A-Za-z0-9]/g, ''); // Remove non-alphanumeric characters
}
...
app.get('/api/auth/discord/callback', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: userData.id, username: userData.username }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
app.get('/api/restricted', validateJWT, (req, res) => {
    res.json({ message: 'Welcome to the restricted route!', user: req.user });
});
...
app.get('/api/testgpt', async (req, res) => {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Error from API endpoint: ${await response.text()}`);
        const data = await response.json();
        res.json(data);
    } catch (error) {
...
app.get('/api/webhooksend', async (req, res) => {
    ...
    if (!title || !description || !color) return res.status(400).json({ error: 'Title, description, and color are required.' });

    const embedMessage = { title, description, color: parseInt(color, 10) };
    try {
        ...
        if (!response.ok) throw new Error(`Error sending webhook: ${await response.text()}`);
